...
/*   Updated: 2026/09/10 15:00:04 by joshtan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *  terminating NUL byte shall be appended to the result, unless its location would be at or beyond dst + dstsize. 
 ...
 * @param size the total size of the destination buffer, meaning the full capacity allocated for the string including space for the null terminator 
 * @return size_t 
 *  - initial length of the string (if any) pointed to by dst, as limited by dstsize
 *  - plus the length of the string pointed to by src; that is, the value that would
 *  -  be returned by strnlen(dst, dstsize) + strlen(src) before the strlcat() call.
 */
